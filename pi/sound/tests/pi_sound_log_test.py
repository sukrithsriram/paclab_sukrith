...
# Creating a ZeroMQ context and socket for sending current parameters 
param_context = zmq.Context()
param_socket = param_context.socket(zmq.DEALER)
...
# Param socket
router_ip3 = "tcp://192.168.0.207:5557"
param_socket.connect(router_ip3) 
print(f"Connected to router at {router_ip3}")  # Print acknowledgment
...
try:
    ...
    while True:
        ...
        if json_socket in socks and socks[json_socket] == zmq.POLLIN:
            ...
            # Construct a string containing the current parameters
            current_params_str = f"{pi_identity} Params - PWM Frequency: {pwm_frequency}, PWM Duty Cycle: {pwm_duty_cycle}, Chunk Min: {chunk_min}, Chunk Max: {chunk_max}, Pause Min: {pause_min}, Pause Max: {pause_max}, Amplitude Min: {amplitude_min}, Amplitude Max: {amplitude_max}"
            
            # Send the current parameters string to the connected IP address
            try:
                param_socket.send_string(current_params_str)
                print("Current parameters sent:", current_params_str)
            except Exception as e:
                print("Error sending current parameters:", e)
