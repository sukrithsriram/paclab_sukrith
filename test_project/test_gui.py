...
class PiWidget(QWidget):
    ...
    def start_sequence(self):
        # Starting the timer to play a sound every 2 seconds
    ...
    def update_Pi(self):
        ...
        # Setting a random Pi to green
        green_Pi = random.choice(self.Pi_signals)
        green_Pi.set_green()

        # Determining the number of the Pi playing audio
        number = int(green_Pi.label.toPlainText().split('-')[-1])

        # Recording the sequence in which the Pis play audio
        self.green_Pi_numbers.append(number)

        # Sending the Pi value to MainWindow for plotting
        self.main_window.plot_green_pi(number)
        self.main_window.plot_window.update_signal(self.green_Pi_numbers[-1])
